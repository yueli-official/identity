...
func EncodeState(secret []byte, returnTo, nonce, bind string, exp int64) string {
	return EncodeProviderState(secret, "", returnTo, nonce, bind, exp)
}

func EncodeProviderState(secret []byte, provider, returnTo, nonce, bind string, exp int64) string {
	p, _ := json.Marshal(statePayload{Provider: provider, RT: returnTo, Nonce: nonce, Bind: bind, Exp: exp})
...
func DecodeState(secret []byte, token string) (returnTo, nonce, bind string, err error) {
	_, returnTo, nonce, bind, err = DecodeProviderState(secret, token)
	return returnTo, nonce, bind, err
}

func DecodeProviderState(secret []byte, token string) (provider, returnTo, nonce, bind string, err error) {
	...
	return p.Provider, p.RT, p.Nonce, p.Bind, nil
}
