...
import {
  decodeWithSealKeyRing,
  encodeProductSession,
} from "../../utils/product-session";
...
export default defineEventHandler(async (event) => {
  ...
  const tx = decodeWithSealKeyRing<Tx>(
    getCookie(event, cfg.cookies.transaction),
    cfg,
  );
  ...
  setCookie(event, cfg.cookies.session, encodeProductSession(session, cfg), {
    httpOnly: true,
