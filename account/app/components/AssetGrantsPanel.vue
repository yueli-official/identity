...
import { ManageEmpty, ManagePagination } from '@platform/manage/components'
...
function status(grant: AssetGrant): { label: string, color: 'success' | 'warning' | 'neutral' } {
...
<template>
  <section class="space-y-4" aria-labelledby="asset-grants-heading">
    <div>
      <h2 id="asset-grants-heading" class="text-sm font-semibold text-highlighted">交付授权</h2>
      <p class="mt-1 text-xs text-muted">查看私有素材的签发用途、使用次数、有效期与撤销状态。</p>
    </div>
    ...
    <div v-else class="overflow-hidden rounded-lg border border-default bg-default">
      <div v-for="grant in grants" :key="grant.id" class="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 border-b border-default px-4 py-3 last:border-b-0">
        ...
        <div class="min-w-0">
          <div class="flex min-w-0 flex-wrap items-center gap-1.5">
            <span class="truncate text-sm font-medium text-highlighted">{{ grant.reason || grant.policy }}</span>
            <UBadge :label="status(grant).label" :color="status(grant).color" variant="soft" size="sm" />
            <UBadge :label="grant.purpose || 'delivery'" color="info" variant="soft" size="sm" />
            <UBadge :label="grant.policy" color="neutral" variant="soft" size="sm" />
          </div>
          <p class="mt-0.5 truncate font-mono text-xs text-muted">{{ grant.assetId }}</p>
        ...
        <UDropdownMenu :items="actionsFor(grant)">
          <UButton icon="i-tabler-dots-vertical" color="neutral" variant="ghost" square size="sm" :aria-label="`授权操作：${grant.id}`" />
        </UDropdownMenu>
      </div>
    </div>
    <div v-if="total" class="flex flex-wrap items-center justify-between gap-3 text-sm text-muted">
