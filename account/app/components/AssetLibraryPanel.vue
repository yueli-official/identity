...
import type { CollectionControl, CollectionControlValue, CollectionPanelMessages, CollectionPanelState } from '@yueli/ui/collection'
import { CollectionPanel } from '@yueli/ui/collection/pattern'
import { ManageViewToggle } from '@platform/manage/components'
  ...
  <section class="space-y-4" aria-label="中央素材库">
    ...
    >
      ...
      <template #bulk-actions>
        <span class="hidden text-xs text-muted sm:inline">仅图片会处理，其它类型记录为未完成</span>
        <UButton
      ...
      <template #item="{ item: asset }">
        <div v-if="view === 'grid'" class="-m-4 flex min-h-full flex-col overflow-hidden rounded-lg">
          ...
          <div class="min-w-0 p-3">
            ...
            <div class="mt-3 flex items-end justify-between gap-3 border-t border-default pt-2.5 text-xs text-muted">
              <div class="min-w-0">
                <p class="truncate">
                  {{ formatBytes(asset.size) }}<span v-if="asset.width && asset.height"> · {{ asset.width }}×{{ asset.height }}</span>
                </p>
        ...
        <div v-else class="grid min-w-0 gap-3 sm:grid-cols-[3.5rem_minmax(0,1fr)] md:grid-cols-[3.5rem_minmax(0,1fr)_11rem_auto] md:items-center">
          <div class="hidden size-14 place-items-center overflow-hidden rounded-lg bg-elevated sm:grid">
            <img
              v-if="asset.cdnUrl && asset.mime.startsWith('image/')"
              :src="asset.cdnUrl"
              :alt="asset.filename"
          ...
          <div class="min-w-0">
            ...
            <p class="mt-0.5 truncate text-xs text-muted">
              {{ asset.mime }} · {{ formatBytes(asset.size) }}<span v-if="asset.width && asset.height"> · {{ asset.width }}×{{ asset.height }}</span>
            </p>
          ...
          <div class="flex justify-end">
            <UDropdownMenu :items="actionsFor(asset)"
              ><UButton icon="i-tabler-dots-vertical" color="neutral" variant="ghost" square size="xs" :aria-label="`素材操作：${assetLabel(asset)}`"
            /></UDropdownMenu>
          </div>
        </div>
      </template>
    ...
    <UAlert v-if="queueError" color="error" variant="subtle" icon="i-tabler-alert-circle" title="无法创建重建任务" :description="queueError" />
